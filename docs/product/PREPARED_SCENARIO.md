...
| Complaint volume is not a demand input.                 | Fixed                                                 | All tracks        | Tests prove a complaint-shaped field is refused by the planner. They cannot prove that a 311 *value* under an innocent name changes nothing — that was falsified. The artifact validator requires every `planning_load` to declare a derivation it recomputes against the published forecast                     |
