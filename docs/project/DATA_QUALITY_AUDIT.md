...
| DSDP April/June 2026 public report              | Monitoring lane only          | Public post-freeze aggregate checkpoints; same publisher, irregular cadence, and no January observation, so not model validation |
| SDHC dashboard                                  | System-context lane           | City-program HMIS outcomes are not downtown observations, population, live capacity, or need                                   |
| HUD CA-601 performance/PIT/HIC                  | System-context lane           | Official annual countywide definitions and outcomes; never resampled into the monthly forecast or planner                      |
...
| DSDP archive and sandiegodata-projects repos    | Provenance only               | Public reports and digitization/update lineage, not an independent observation stream; ingest pinned documents, not archive HTML |
