...
## Spacing is left alone, because the measurement said so

The type scale was applied because two things were true at once: the sheet
carried 55 distinct sizes with 26 of them inside a three-pixel band, which is
proof they were nudged rather than chosen, and a 1.1 ratio collapsed them to 18
while moving nothing more than 1.15px. Invisible change, dramatic gain.

Spacing was measured the same way and neither holds. 43 distinct rem values
across 386 uses, and the crowded band is not crowded in the same sense: the
common values are reused heavily — 0.5rem 29 times, 0.8rem 26, 0.7rem 23,
0.65rem 21, 0.35rem 17 — which is what deliberate reuse looks like, not what
eyeballing looks like.

What consolidation costs:

| candidate | steps | max shift | uses moved |
|---|---|---|---|
| 0.125rem grid | 48 | 1.2px | 201 |
| 0.25rem grid | 24 | 3.2px | 201 |
| sub-pixel merges only | 36 | 0.8px | 36 |

The only change that is invisible is also nearly pointless: seven merges over 36
uses, 43 distinct values down to 36. Everything with real reach moves 201
declarations by up to 3.2px, on a rendering nobody involved has ever seen. A 1px
shift in a font size is below the threshold of sight; a 3px shift in a gap is
not. So: nothing applied, and the measurement recorded here so the next person
does not spend the afternoon rediscovering it.

**A caution about the measurement itself.** The first candidate scales were
generated by keeping every value used at least N times and snapping the rest to
the nearest keeper. That truncates the top of the range: with no keeper above
1.5rem, a 5rem padding "snapped" to 1.5rem and the table reported a 56px shift
as though it were a design option. The same shape of error had already appeared
twice — a radius sweep that snapped the 999px pill idiom to 22px and reported a
977px shift, and a `--pytest-summary` that returned "unreconciled" while
appearing to reconcile. Three instances in one session of a computation that
runs cleanly, looks authoritative, and is wrong. The recurring defect in this
repository is not code that fails; it is checks and measurements that succeed
without earning it.
