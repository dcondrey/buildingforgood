...
- **R-11:** the caption then read "simplified neighborhood boundaries, aggregate values only · not a count of people". **Superseded 2026-08-23:** that wording implied a real boundary that had been simplified, and no such boundary is published. Every caption now carries the locale's registered `map.outlinePhrase` — "schematic outlines, not surveyed boundaries" — enforced by `i18n.test.tsx`. The tabular equivalents and non-color state words are unchanged; the map areas are additionally keyboard-selectable buttons feeding a text detail panel, which strengthens the non-color path. Still resolved.
...
**2026-08-21 update (digitization-audit card and guide step).** The field-sheet digitization audit now renders in the drop-test section and as guide step 9. Full-sentence read of the new strings: every value is framed as recovered text from an already-published aggregate document; the reconciliation sentence (152 + 14 × 1.75 = 176.5 ≈ 177) is arithmetic over publisher-stated multipliers, not a claim about people; OCR instability is stated in the same block ("candidates for human verification, never counts"); the per-page table carries only integer-token counts, area-scale values, and withheld counts, mirroring the committed card's C-02 filter; and the engine chip names the local offline engine or the hosted drop-in without implying sponsor endorsement. "Vision that audits the instrument, never the people" is a boundary statement, not a capability claim. No finding.

*Addendum (2026-08-21, cross-resolution agreement).* The audit card's finding paragraph was rewritten ("Recovered, misread, and caught") and a "Reading-vs-reading agreement" panel plus a fifth falsification bullet were added; every new rendered string received the full-sentence read. The finding paragraph now states the shipped card's 157 against the sheet's 152 — both are area-scale values recovered from an already-published aggregate document, so no boundary change. The agreement panel and its per-page table write only values and counts already present in the two privacy-filtered committed cards (shared/exclusive counts, engine and DPI provenance, agreement share); its closing sentence ("same engine read twice is a floor on digitization instability, not an independent second opinion") is a methods caveat with readings as its subject. The new falsification bullet ("two readings currently disagree on 2.5% of recovered values") quantifies instrument error, never a claim about people. No finding.

**2026-08-21 update (pending-requests disclosure).** One new rendered string in the Data & limits drawer names the four agencies holding our pending data requests and commits responsive records to the ledger's documented lanes before analytical use. Full-sentence read: factual, procedural, no population claim, no implication that any agency endorses the product. No finding.

**2026-08-21 update (map workspace view).** A second, opt-in view ("Map workspace" / "Story" toggle) presents one layered area map (Planned hours / Observed change / Unmet load) with a tabbed inspector that reuses the planner, workbench, explorer, and brief blocks verbatim, so previously adjudicated strings render unchanged. New rendered strings received the full-sentence read: layer labels and captions keep hours/observed-units as subjects and carry "not a count of people" adjacent on every layer; the unmet layer's state words ("Load moved by minimums", "Follows forecast") apply movement to hours/load only; the unified Area dossier repeats existing adjudicated rows plus procedural hints ("hours moved by minimums and locks", "current plan minus pinned scenario"); the assumption explorer appears in the dossier with its full disclosure block intact, and the map caption carries the "modeled as cleared (assumption)" note in this view too, preserving the adjacency condition. The workspace shows the same aggregate area values as the story view — no new data surface, no block geometry. No finding.

**2026-08-21 update (opens-mid-work shell).** The shell now opens with the default 80h/8h plan already computed, so the "Generate coverage scenario" reveal only appears if a plan is cleared. New rendered strings received the full-sentence read: the sticky-nav live plan state chips ("N/Nh allocated", "Nh floor", "Nh unmet load", "N locks", "assumption: <Area> cleared") are procedural with hours as subject — the assumption chip repeats the explorer's label and appears only while the fully-disclosed banner is also on screen, satisfying the adjacency condition; and the guide's plan step was rewritten ("The plan is already on the table") with no new population claim. No finding.
...
**2026-08-21 update (capacity-context card, #79).** The brief cluster gained a collapsed "Capacity context: staff-hours in staffing terms" details card in both views. Every rendered sentence received the full-sentence read against §4's eligibility, dispatch, and person-claim rules. The card's subjects are hours, the budget, guidance, and benchmarks — never people: the budget restatement names both the budget and the forty-hour week as "stated assumptions, not staffing data"; the cited ratios are attributed in-sentence to HUD's Case Management Ratios guidance and immediately scoped ("describes community-based case management, not street outreach"), with the absence of any street-outreach standard in primary federal guidance stated rather than papered over, and the guidance labeled "planning help, not binding rules"; the closing sentence carries the exclusion and the boundary in one breath ("No benchmark number enters the allocation, and nothing here estimates any person's service need, eligibility, or availability"). The card renders no number without its citation adjacent, is collapsed by default, and is context-only — no benchmark value reaches the planner, the forecast, or any rendered allocation figure. Full source provenance, including the rejected HUD-VASH 1:25 folk figure (an oversight-report staffing average, not directive text), is recorded in the `hud_case_management_ratios` ledger card. No finding.

**2026-08-21 update (local datum on the capacity-context card).** One paragraph added between the HUD guidance and the exclusion sentence, rendering the contracted case-management ratios from the pinned City PRA release (Alpha Project shelter proposal, `city_pra_capacity_records` ledger card). Full-sentence read: the sentence's subjects are the release, the proposal, and the ratios — never people; the ratios are attributed in-sentence and the caveat is adjacent in the same paragraph, not behind a control ("proposed shelter staffing ratios, not street outreach and not observed practice"); "2023-era" avoids implying a current contract; the existing exclusion sentence ("No benchmark number enters the allocation…") still closes the card. Verified against the raw extract before rendering: "at a ratio of 1:12.5" (families, 4.0 FTE) and "at a ratio of 1:15" (single adults, 16.0 FTE) appear verbatim in the pinned PDF, checksum matching the ledger pin. No finding.

**2026-08-21 update (hero and scorecard labeling, external review).** An external reviewer flagged the hero as undercutting the product's own discipline, and the fix landed: the lede now names its series and denominator ("component-derived unsheltered estimate fell 22% in a year on the fixed 261-block panel") and replaces "outreach workers saw more people" with observation-subject language ("direct observations of people rose and appeared on 25 more blocks"), matching the adjudicated constructions; the README lede was aligned the same way and the stage-script hook now says "adjusted unsheltered estimate". The model scorecard names its two denominators so the coexisting error levels stop reading as a bug: the audit tiles are labeled "2025 audit MAE/WAPE", the table columns "2023 MAE/2023 WAPE", and the promotion-rule sentence states that the scorecard is the 2023 promotion window while the audit row is the untouched 2025 walk-forward. Full-sentence read on every changed string; all subjects are estimates, observations, and error metrics, never people. No finding.
