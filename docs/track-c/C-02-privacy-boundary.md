...
Detection is **context-based, not name-based**: any object carrying an area and/or period key (`neighborhood`, `area_id`, `month`, `period`, `date`) is a cell, and every integer inside it — and inside its children — is a published cell value. An integer `0 < v < min_cell` is a **BLOCK**. Default threshold is 5, configurable via `--min-cell`. Zero is permitted, and a short allow-list exempts structural integers (`schema_version`, `horizon_periods`, `time_increment`, …). The escape hatch is to publish the cell as suppressed — a sibling `suppressed`/`redacted` marker clears the rule — and per R-06 the suppression must be visible in the UI as a data-quality state, never rendered as a zero.
